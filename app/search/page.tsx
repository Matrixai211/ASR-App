...
export default function Search() {
  ...
  return <main>
    ...
    {searched && <section className="grid search-results">
      {results.length ? results.map(item => <a className="card search-card" href={item.href || '#'} key={item.id}>
        {item.artwork ? <img className="cover cover-img" src={item.artwork} alt={`${item.title} artwork`} /> : <div className="cover">ASR</div>}
        <div className="tag">{item.type.toUpperCase()}</div>
        <h3>{item.title}</h3>
        <p>{item.subtitle}{item.genre ? ` · ${item.genre}` : ''}</p>
      </a>) : !loading && <div className="search-empty"><div className="eyebrow">NO RESULTS</div><h2>Nothing matched “{query}”.</h2><p>Try an artist, track, album or genre.</p></div>}
    </section>}
  </main>;
}
